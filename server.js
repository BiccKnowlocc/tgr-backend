...
// + FULL admin UI + endpoints (list/view/status/cancel/delete/export)
...
//
// NOTE: Order IDs restored as random 6-digit IDs: TGR-123456 (unique with retry)
...
// Square pay links (used on member portal buttons)
...
// ===== Robust random order ID generator: TGR-123456 =====
async function nextOrderId() {
  // 6-digit numeric; retry if collision
  for (let i = 0; i < 12; i++) {
    const n = crypto.randomInt(0, 1000000);
    const candidate = "TGR-" + String(n).padStart(6, "0");
    const exists = await Order.exists({ orderId: candidate });
    if (!exists) return candidate;
  }
  // fallback: add 2 more random digits
  const n = crypto.randomInt(0, 100000000);
  return "TGR-" + String(n).padStart(8, "0");
}
  ...
  <div class="card">
    ...
    <div class="grid">
      ...
      <div>
        ...
        <div class="row" style="margin-top:10px;">
          <div style="flex:1 1 260px;"><input id="q" placeholder="e.g., TGR-123456 or Bullock or Tobermory"></div>
...
<script>
  ...
  async function fetchOrders(){
    ...
    items.forEach(o=>{
      ...
      tr.innerHTML = \`
        ...
        <td>
          ...
          <div class="muted" style="font-size:12px;">\${o.customer?.phone || "—"} • \${o.customer?.email || "—"}</div>
        </td>
        <td>
        ...
        <td>
          ...
          <div class="muted" style="font-size:12px;">\${o.address?.streetAddress || "—"} \${o.address?.unit ? (" • " + o.address.unit) : ""} • \${o.address?.postalCode || ""}</div>
        </td>
        <td>
    ...
    tbody.querySelectorAll("[data-view]").forEach(btn=>{
      btn.addEventListener("click", ()=> viewOrder(btn.getAttribute("data-view")));
    });
    tbody.querySelectorAll("[data-cancel]").forEach(btn=>{
      btn.addEventListener("click", ()=> cancelOrder(btn.getAttribute("data-cancel")));
    });
    tbody.querySelectorAll("[data-del]").forEach(btn=>{
      btn.addEventListener("click", ()=> deleteOrder(btn.getAttribute("data-del")));
    });
